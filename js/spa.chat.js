...
spa.chat = (function() {
  ...
  setJqueryMap = function() {
    ...
    jqueryMap = {
      ...
      $toggle   : $slider.find( '.spa-chat-head-toogle' ),
  ...
  // 要求された位置が全芸の位置の場合には、何もせずに true を返す。
  // 戻り値:
  //   * true - 要求された位置に移動した
  ...
  // チャットメッセージを管理するプライベート DOM メソッド開始
  scrollChat = function() {
  ...
  // チャットメッセージを管理するプライベート DOM メソッド終了
  //---------------- DOM メソッド終了 ------------------------
  ...
  onSubmitMsg = function( event ) {
    ...
    setTimeout(
      function() { jqueryMap.$send.revemoClass( 'spa-x-select' ); },
      250
    );
    return false;
  };
  ...
  // 動作:
  //   指定されたコンテナにチャットスライダーを付加し、HTML コンテンツで埋める。
  //   そして、要素、イベントハンドラを初期化し、ユーザにチャットルームインターフェースを提供する。
  ...
  initModule = function( $append_target ) {
    ...
    // チャットスライダー HTML と jQuery キャッシュをロードする
    ...
    // $list_box で jQuery グローバルイベントに登録する
  ...
  // パブリックメソッド/revemoSlider/開始
  // 目的:
  //   * DOM 要素 chatSlider を削除する
  //   * 初期状態に戻す
  ...
  removeSlider = function() {
    // 初期化と状態を解除する
    // DOM コンテナを削除する。これはイベントのバインディングも削除する。
    if ( jqueryMap.$slider ) {
